...
type DepfactryFunc func(c Container) (any, error)

type tempValue = chan struct{}

type Container interface {
	AddSingleton(key string, fn DepfactryFunc)
	AddScoped(key string, fn DepfactryFunc)
...
func (c *container) AddScoped(key string, fn DepfactryFunc) {
	c.deps[key] = depInfo{
		key: key,
		scope: Scoped,
		factory: fn,
...
func (c *container) AddSingleton(key string, fn DepfactryFunc) {
	c.deps[key] = depInfo{
		key: key,
		scope: Singleton,
		factory: fn,
